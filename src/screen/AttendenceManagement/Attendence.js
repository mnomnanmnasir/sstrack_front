...
const Attendence = () => {
    ...
    const fetchAttendanceData = async (date) => {
        ...
        try {
            const formattedDate = date.toISOString().split('T')[0];
            const response = await axios.get(`${apiUrl}/owner/getAttendanceRecord?date=${formattedDate}`, { headers });
            if (response.data.success) {
                setAttendanceData(response.data.onlineUsers);
            }
        } catch (error) {
                        ...
                        <table className="table align-middle text-center" style={{ borderSpacing: "0", borderCollapse: "separate" }}>
                            ...
                            {attendanceData.map((row, index) => (
                                    ...
                                    <tr key={index} style={{ borderBottom: "1px solid #E5E7EB" }}>
                                        ...
                                        <td style={{ color: "#374151", fontSize: "14px" }}>Karachi Pak</td>
                                        <td style={{ color: "#374151", fontSize: "14px" }}>1001</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
